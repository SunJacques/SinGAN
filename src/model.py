...
class ConvBlock(nn.Sequential):
    def __init__(self, in_channel, out_channel, ker_size, padd, stride, padding_mode='zeros'):
        super(ConvBlock,self).__init__()
        self.add_module('conv',nn.Conv2d(in_channel ,out_channel, kernel_size=ker_size, stride=stride, padding=padd,padding_mode=padding_mode)),
...
class Generator (nn.Module):
    def __init__(self, opt, scale):
        ...
        self.head = ConvBlock(in_channel=3, out_channel=N_init, ker_size=3, padd=5, stride=1, padding_mode='reflect')
